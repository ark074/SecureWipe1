...
async fn start_wipe(req: web::Json<WipeRequest>, data: web::Data<AppState>) -> impl Responder {
    ...
    thread::spawn(move || {
        ...
        if used_method.is_empty() {
            ...
            let mut f = match File::options().write(true).open(p) {
                Ok(x) => x,
                Err(e) => {
                    let mut jobs = state.jobs.lock().unwrap();
                    if let Some(j) = jobs.iter_mut().find(|j| j.id==id) {
                        j.status = "failed".into();
                        j.message = format!("failed to open target: {}", e);
                        j.progress = 0.0;
                    }
                    error!("Failed to open target {}: {}", path, e);
                    return;
                }
            };
        ...
        py_cmd.arg("../tools/sign_and_send.py").arg("--cert").arg(&cert_path).arg("--email").arg(&r.email);
...
async fn main() -> std::io::Result<()> {
    ...
    if std::fs::read_to_string("./frontend/index.html").is_err() {
